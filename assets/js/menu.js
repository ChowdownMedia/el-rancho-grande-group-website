/* Mi Jalapeno menu board: search, filters, surprise me, item sheet.
   Runs against a prebuilt compact index (window.MENU_INDEX). Deferred. */
(function () {
  "use strict";
  var IDX = window.MENU_INDEX || [];
  // per-location menu: index urls are relative (cat/#item); prefix with the current location.
  var LOC = (location.pathname.match(/^\/([a-z0-9-]+)\/menu\//) || [])[1];
  var U = function (u) { return (LOC ? "/" + LOC + "/menu/" : "/menu/") + u; };
  ...
  function render() {
    ...
    var rows = hits.map(function (it) {
      ...
      return '<a class="rrow" href="' + U(it.u) + '">' +
